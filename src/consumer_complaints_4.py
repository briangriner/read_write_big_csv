...
if __name__ == "__main__":
    ...
    writer = csv.writer(wdata, quoting=csv.QUOTE_ALL)
    # manually write header row
    writer.writerow(("Product","Date_received","Company"))
    # call gen - read row - select fields - write row to file
    for row in read_big_csv(indata):
        prod_date_company = (row[1], row[0], row[7].lower())
